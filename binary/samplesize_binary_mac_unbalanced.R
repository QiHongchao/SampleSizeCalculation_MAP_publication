##The script must be executed after the execution of "samplesize_binary_nb.R"
rm(list=ls())
...
for (k in 1:length(OR_candidate)) {
  OR <- OR_candidate[k]
  ##Specify the sample size of the treatment arm
  samplesize_treatment <- samplesize_treatment_candidate[k]
for (j in 1:samplesize_num) {
  ##Specify the sample size of the control arm
