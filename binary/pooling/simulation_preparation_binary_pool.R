...
##Pooling
jags_data_pool <- list(nsub = sum(nsub_historical), event = sum(event_historical))

pool_jags_model <- jags.model(file = "./jags/pool_binary_historical.txt", data = jags_data_pool, 
                              n.chains = num_chains, n.adapt = 1000, quiet = T,
                              inits = list(list(.RNG.name = "base::Wichmann-Hill", .RNG.seed = seeds_jags[1]),
                                           list(.RNG.name = "base::Wichmann-Hill", .RNG.seed = seeds_jags[2]),
                                           list(.RNG.name = "base::Wichmann-Hill", .RNG.seed = seeds_jags[3]),
...
##Sample size based on the sample size formula
p0_mean <- mean(pool_sample$p0)
p0_median <- median(pool_sample$p0)
samplesize_calc <- function(p0, OR) {
  p1 <- OR*p0/(1+(OR-1)*p0)
  (qnorm(0.975) + qnorm(0.8))^2 * (p0 * (1 - p0) + p1 *(1 - p1))/(p1-p0)^2
}
samplesize_calc(p0_mean, OR_candidate)
samplesize_calc(p0_median, OR_candidate)

##step 7: have a list of candidate sample sizes
samplesize_num <- 6
samplesize_candidate <- c(seq(10, 260, length.out = samplesize_num),
                          seq(100, 400, length.out = samplesize_num),
                          seq(300, 700, length.out = samplesize_num))
interval <- c(50, 60, 80)
